...
shared_examples_for 'pcp::config' do |_facts|
  ...
  it do
    is_expected.to contain_pcp__pmlogger('local').with(primary: 'true',
                                                       args: '-r -T24h10m -c config.default')
  end

  it do
    is_expected.to contain_file('/etc/pcp/pmie/control.d').with(ensure: 'directory',
                                                                owner: 'root',
                                                                group: 'root',
                                                                mode: '0755',
                                                                recurse: 'true',
  ...
  it do
    is_expected.to contain_pcp__pmie('local').with(args: '-c config.default')
  end

  it do
    is_expected.to contain_file('/etc/cron.d/pcp-pmlogger').with(ensure: 'file',
                                                                 owner: 'root',
                                                                 group: 'root',
                                                                 mode: '0644')
  end
  ...
  it do
    is_expected.to contain_file('/etc/cron.d/pcp-pmie').with(ensure: 'file',
                                                             owner: 'root',
                                                             group: 'root',
                                                             mode: '0644')
  end
